...
if __name__ == "__main__":
    ...
    log_path = os.path.join(log_prefix, experiment_name + "_train")
    ...
    if selection_type in ["DiffShapley"]:
        all_sample_feature_maps = compute_all_sample_feature_maps(dataset, train_data, model_A, model_B, n_classes, window_length, selection_M, flag='train', device=device)

    if augmentation_type in ["Counterfactual"]:
        aug = counterfactual(model_A, model_B, dataset, train_data, cover=False, device=device, target_model=1, flag='train')# if model_A_type != 'rf' else 2)
    ...
    for explainer_type in explainer_types:
        ...
        x_train_aug, delta_target_aug = augmentation_method.augment(x_train, delta_target, augment_factor=augment_factor, )
        if augmentation_type == "Counterfactual":
            if len(aug.shape) == 3:
                x_train_aug = np.concatenate((x_train_aug, aug), axis=0)
            elif len(aug.shape) == 4:
                x_train_aug = np.concatenate((x_train_aug, aug[:, :int(augment_factor)].reshape(-1, channels, points)), axis=0)
        ...
        if selection_type in ["DiffShapley"]:
            ...
            selection_method.fit(x_train, model_A, model_B, channels, points, n_classes,
                                 window_length, selection_M, all_sample_feature_maps,
        ...
        if explainer_type in ["Logit"]:
            ...
            test_metrics = explainer.metrics(x_test, output_A, output_B)
        else:
            train_metrics = explainer.metrics(x_train, pred_target_A_train, pred_target_B_train, name="train")
            test_metrics = explainer.metrics(x_test, pred_target_A, pred_target_B)
