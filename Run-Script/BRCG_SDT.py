...
from aix360.algorithms.rbm import BRCGExplainer, FeatureBinarizer
...
if __name__ == "__main__":
    ...
    val_loader = get_data_loader_from_dataset('../dataset/{}_test.npz'.format(cfg.DATASET.TYPE), 128)
    ...
    X_train_fb = fb.fit_transform(pd.DataFrame(data.cpu().detach().numpy().reshape((-1, 204*100))))

    BRCG = BRCGExplainer()
    BRCG.fit(X_train_fb, pred_target_A)
    # imd.fit(pd.DataFrame(data.cpu().detach().numpy().reshape((-1, 204 * 100))), pred_target_B)

    trxf_ruleset = BRCG.explain()
