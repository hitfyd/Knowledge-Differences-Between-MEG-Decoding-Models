...
from .layer import Flatten, DepthwiseConv2d, SeparableConv2d, Conv2dWithConstraint
...
class CustomNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        if self.__class__.__name__ == "MEEGNet":
            _, channels, points = x.shape
            x =x.reshape(-1, channels // self.physical_channels, self.physical_channels, points)
...
def meegnet(channels=204, points=100, num_classes=2, physical_channels=102, n_linear=2000, **kwargs):
    S = channels // physical_channels
    return MEEGNet(input_size=(S, physical_channels, points), n_outputs=num_classes, physical_channels=physical_channels, n_linear=n_linear)
